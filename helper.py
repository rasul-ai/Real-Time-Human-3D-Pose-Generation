...
def model_load(config):
    ...
    # Update to handle CPU-only machines
...
def hrnet_pose(frame, det_dim=416, num_peroson=1, gen_output=False):
    ...
    # Check the shape of the keypoints array before transposing
    if keypoints.ndim == 4:  # Make sure the shape is (T, M, N, 2)
        keypoints = keypoints.transpose(1, 0, 2, 3)  # (T, M, N, 2) --> (M, T, N, 2)
    else:
        print(f"Unexpected shape of keypoints: {keypoints.shape}")
        return None, None
